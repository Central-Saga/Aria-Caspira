...
new class extends Component {
    ...
    public NotifikasiStok $notif;
    public int $baju_id;
    public string $status = 'warning';
    public ?string $pesan = null;
    public bool $terbaca = false;

    public function mount(NotifikasiStok $notifikasi)
    {
        $this->notif = $notifikasi;
        $this->baju_id = $notifikasi->baju_id;
        $this->status = $notifikasi->status;
        $this->pesan = $notifikasi->pesan;
        $this->terbaca = (bool) $notifikasi->terbaca;
    }
    ...
    public function update()
    {
        ...
        $this->notif->update($validated);

        session()->flash('message', __('Notifikasi diperbarui.'));
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="mb-4 sm:mb-0">
                    <h1 class="text-4xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">{{ __('Edit Notifikasi Stok') }}</h1>
                    <p class="mt-2 text-lg text-gray-600 dark:text-gray-300">{{ __('Atur status dan pesan notifikasi') }}</p>
                </div>
