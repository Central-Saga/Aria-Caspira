...
new class extends Component {
    ...
    public ?KategoriBaju $kategoriToDelete = null;
    ...
    {
        try {
            $this->kategoriToDelete = KategoriBaju::findOrFail($kategoriId);
    ...
    public function deleteKategori(int $kategoriId): void
    {
        try {
            $kategori = KategoriBaju::findOrFail($kategoriId);
            $kategori->delete();
    ...
    public function with(): array
    {
        ...
        return [
            'categories' => $query->orderBy('nama_kategori')->paginate(10),
            'stats' => [
                'totalCategories' => KategoriBaju::count(),
            ],
        ];
    }
}; ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400 truncate">{{ __('Total Categories') }}</p>
                        <p class="text-3xl font-bold text-gray-900 dark:text-white mt-2 truncate">{{ $stats['totalCategories'] }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse($categories as $category)
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all duration-300 overflow-hidden group">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-center space-x-3 min-w-0 flex-1">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400 truncate">ID: {{ $category->id }}</p>
                                </div>
                            </div>
                    ...
                    <div class="p-6">
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Dibuat pada') }}: {{ $category->created_at?->format('d M Y H:i') }}</p>
                    </div>
                </div>
            @empty
                <div class="col-span-full">
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('No categories found') }}</h3>
                            <p class="text-gray-500 dark:text-gray-400">{{ __('Get started by creating a new category.') }}</p>
                        </div>
                    </div>
                </div>
            @endforelse
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    @if($kategoriToDelete)
                                        ...
                                        <strong class="text-gray-900 dark:text-white">{{ $kategoriToDelete->nama_kategori }}</strong>?
                    ...
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3">
                        <button wire:click="deleteKategori({{ $kategoriToDelete ? $kategoriToDelete->id : 0 }})"
                                type="button"
